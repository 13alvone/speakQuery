...
cpp_datetime_path = os.path.join(
    project_root, "functionality", "cpp_datetime_parser", "build"
)
...
try:
    cpp_datetime_module = resolve_and_import_so(
        cpp_datetime_path, "cpp_datetime_parser"
    )
...
class speakQueryListener(ParseTreeListener):
    ...
    def exitLoadjobInit(self, ctx: speakQueryParser.LoadjobInitContext):
        ...
        self.main_df = self.general_handler.loadjob_pickle_file(
            self.current_loadjob_path
        )
        self.validate_exceptions(ctx)
