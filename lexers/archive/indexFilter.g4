...
query
    : indexFilter validLine* (EOF | NEWLINE?)
    ;

indexFilter
    : LPAREN indexFilter RPAREN
    | (filterClause (NEWLINE+ filterClause)*)
    | inputlookupInit
    ;

filterClause
    : earliestClause latestClause? filterClause
    | latestClause earliestClause? filterClause
    | filterClause earliestClause latestClause?
    | filterClause latestClause earliestClause?
    | indexClause
    | logicalExpr
    | comparisonExpr
    | inExpression
...
logicalOrExpr
    : NOT? NEWLINE* LPAREN logicalOrExpr RPAREN
    | logicalAndExpr (NEWLINE* OR NEWLINE* logicalAndExpr)*
    ;

logicalAndExpr
    : NOT? NEWLINE* LPAREN logicalAndExpr RPAREN
    | logicalPrimaryExpr (NEWLINE* AND NEWLINE* logicalPrimaryExpr)*
    | logicalPrimaryExpr (NEWLINE* logicalPrimaryExpr)*
    ;

logicalPrimaryExpr
    : LPAREN NEWLINE* logicalPrimaryExpr NEWLINE* RPAREN
    | indexClause
    | comparisonExpr
    | inExpression
    ;
...
directive
    : SEARCH (NOT? NEWLINE* | NEWLINE* NOT?) (logicalExpr COMMA?)+
    | WHERE (NOT? NEWLINE* | NEWLINE* NOT?) (logicalExpr COMMA?)+
