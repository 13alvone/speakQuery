...
class CustomTradeQueryListener(tradeQueryListener):
    ...
    def exitTradeQuery(self, ctx: tradeQueryParser.TradeQueryContext):
        ...
        # self.general_handler.add_variables_as_columns(self.variables, self.main_df)
    ...
    def exitDirective(self, ctx: tradeQueryParser.DirectiveContext):
        ...
        if isinstance(last_val, list):
            self.entries.append(''.join([str(x) for x in last_val]))
        else:
            self.entries.append(''.join([str(x) for x in last_val.children]))
        if str(ctx.children[0]) == 'eval':
            if len(ctx.children) == 4:
                # Update the equal_components with data from main_df if found to exist within the headers
                if str(self.current_variable) in self.main_df.columns:
                    self.general_handler.add_or_update_column(self, self.current_variable, ctx.children[-1])
    ...
    def exitExpressionString(self, ctx: tradeQueryParser.ExpressionStringContext):
        ...
        # self.expression_eval(ctx, _type='string')
    ...
    def exitExpressionNumeric(self, ctx: tradeQueryParser.ExpressionNumericContext):
        ...
        # self.expression_eval(ctx, _type='numeric')
    ...
    def exitExpressionAny(self, ctx: tradeQueryParser.ExpressionAnyContext):
        ...
        # self.expression_eval(ctx, _type='specific')
    ...
    def exitStringFunctionCall(self, ctx: tradeQueryParser.StringFunctionCallContext):
        ...
        # target_function_name = str(self.last_expression[0])
        # parameters = self.general_handler.remove_unnecessary_nesting(self.extract_parameters(self.last_expression))
        # if not self.general_handler.are_all_terminal_instances(parameters):
        #     return
        # parameter_length = len(parameters)
        # for index, parameter in enumerate(parameters):
        #     if isinstance(parameter, str) and parameter.lower() in self.main_df.columns:
        #         parameters[index] = self.general_handler.get_column_entries(self.main_df, parameter)
        #
        # if target_function_name in ['upper', 'lower', 'capitalize', 'len', 'defang', 'urlencode', 'urldecode']:
        #     self.last_expression_result[self.concatenate_children(ctx.children)] = \
        #         self.string_handler.transform_strings(parameters, target_function_name)
        #
        # elif target_function_name == 'match':
        #     if parameter_length == 2:
        #         parameters = self.string_handler.validate_string_asts(parameters)
        #         results = self.string_handler.match_strings(parameters[0], parameters[1])
        #         if self.general_handler.are_all_terminal_instances(self.last_expression):
        #             self.last_expression_result[self.concatenate_children(ctx.children)] = results
        #
        # elif target_function_name == 'concat':
        #     results = self.string_handler.full_concat(self.string_handler.validate_string_asts(parameters))
        #     if self.general_handler.are_all_terminal_instances(self.last_expression):
        #         self.last_expression_result[self.concatenate_children(ctx.children)] = results
        #
        # elif target_function_name == 'replace':
        #     if parameter_length == 3:
        #         start_string, string_to_replace, replace_with = self.string_handler.validate_string_asts(parameters)
        ...
        #     if parameter_length == 3:
        #         start_string, start_position, end_position = self.string_handler.validate_string_asts(parameters)
        #         results = self.string_handler.substring(start_string, start_position, end_position)
        #         if self.general_handler.are_all_terminal_instances(self.last_expression):
        #             self.last_expression_result[self.concatenate_children(ctx.children)] = results
        #     else:
        #         logging.error(f'[x] Function "substr" requires 3 parameters. Length {parameter_length} provided.')
        #         exit(1)
        #
        # elif target_function_name in ['trim', 'ltrim', 'rtrim']:
        #     if parameter_length == 2:
        #         results = self.string_handler.trim_strings(self.string_handler.validate_string_asts(parameters[0]),
        #                                                    characters=str(parameters[-1]).strip().strip('"'))
        #         if self.general_handler.are_all_terminal_instances(self.last_expression):
        #             self.last_expression_result[self.concatenate_children(ctx.children)] = results
        #     elif parameter_length == 1:
        #         results = self.string_handler.trim_strings(self.string_handler.validate_string_asts(parameters),
        #                                                    characters=' ')
        #         if self.general_handler.are_all_terminal_instances(self.last_expression):
        #             self.last_expression_result[self.concatenate_children(ctx.children)] = results
        #     else:
        #         logging.error(f'[x] Function "{self.last_expression[0]}" requires 3 parameters. '
        #                       f'Length {parameter_length} provided.')
        #         exit(1)
    ...
    def validate_exceptions(self, ctx_obj, obj_identifier):
        ...
        if obj_identifier.startswith('enter'):
            ...
            # If a terminal node is found, update it within the current stack; then trim it.
            ...
            # elif isinstance(last_key, (TQP.TermNumericResultOnlyExprContext, TQP.TermExprContext)):
            elif isinstance(last_key, TQP.TermExprContext):
                if self.general_handler.are_all_terminal_instances(last_val):
                    results = self.math_handler.perform_mult_div_numpy(self.general_handler.ast_args(last_val))
                    self.replace_list_in_stack_with_results(last_val, results, self.values)
                    last_key, last_val = self.values.popitem(last=True)
                    self.general_handler.add_or_update_column(self, self.current_variable, last_val)
                else:
                    pass

            # Addition/Subtraction Expression:
            # elif isinstance(last_key, (TQP.ArithmeticNumericResultOnlyExprContext, TQP.ArithmeticExprContext)):
            elif isinstance(last_key, TQP.ArithmeticExprContext):
                if self.general_handler.are_all_terminal_instances(last_val):
                    results = self.math_handler.perform_add_sub_numpy(self.general_handler.ast_args(last_val))
                    self.replace_list_in_stack_with_results(last_val, results, self.values)
                    last_key, last_val = self.values.popitem(last=True)
                    self.general_handler.add_or_update_column(self, self.current_variable, last_val)
                else:
            ...
            # Full termination of a line is represented here. This should be a dataframe update/addition section.
            elif isinstance(last_val, list):
                if self.general_handler.are_all_terminal_instances(last_val):
                    if obj_identifier in ['exitDirective']:
                        directive = str(last_val[0])

                        # Initial TIMERANGE filtering is performed here.
                        if directive == 'timerange':
                            parameters = self.extract_parameters(last_val)
                            if len(parameters) != 3:
                                logging.error(f'[x] TIMESTAMP parameters malformed. Received {parameters}')
                                exit(1)
                            start_date, end_date, target = tuple([str(x) for x in parameters])
                            target = self.general_handler.get_column_entries(self.main_df, target)
                            out = self.general_handler.filter_dates(self.time_handler, start_date, end_date, target)
                            self.main_df = self.general_handler.filter_df_with_dates(self.main_df, out)

                        # EVAL statement handling at the Directive Level
                        elif directive == 'eval':
                            last_val = [str(x).strip('"') for x in last_val]

                            # Update the equal_components with data from main_df if found to exist within the headers
                            if len(last_val) >= 4:
                                if last_val[-1] in self.main_df.columns:
                                    last_val[-1] = self.general_handler.get_column_entries(self.main_df, last_val[-1])

                                # Process string values directly and place back on the stack
                                self.general_handler.add_or_update_column(self, str(last_val[1]), last_val[-1])

                        # All String Functions
                        elif directive in self.all_string_funcs:
                            _last_val = [str(x).strip('"') for x in last_val]
                            parameters = self.extract_parameters(_last_val)

                            # Update the equal_components with data from main_df if found to exist within the headers
                            for index, parameter in enumerate(parameters):
                                if parameter in list(self.main_df.columns):
                                    parameter = self.general_handler.get_column_entries(self.main_df, parameter)
                                    if isinstance(parameter, list):
                                        for _index, sub_entry in enumerate(parameter):
                                            parameter[_index] = self.general_handler.validate_ast(sub_entry)
                                    else:
                                        parameter = self.general_handler.validate_ast(parameter)
                                    parameters[index] = parameter

                            parameters = self.general_handler.remove_unnecessary_nesting(parameters)
                            results = last_val[-1]

                            if directive == 'concat':  # Expression stored
                                results = self.string_handler.full_concat(parameters)

                            elif directive in \
                                    ['upper', 'lower', 'capitalize', 'len', 'defang', 'urlencode', 'urldecode']:
                                results = self.string_handler.transform_strings(parameters, directive)
        ...
        # New Directive, Valid Line, Variable
            ...
            if obj_identifier.startswith('enter'):
                ...
                collapsed_values = self.collapse(self, self.values)
                ...
                elif len(self.current_directive) > 1:
                ...
                if isinstance(ctx_obj.children, list):
                    if len(ctx_obj.children) == 1:
                        ctx_children = str(ctx_obj.children[0])
                        if str(ctx_children) in self.main_df.columns:
                            self.variables[ctx_children] = None
                            pass
            else:
                pass
    ...
    def expression_eval(self, ctx_obj, _type):
        ...
        # if str(self.last_expression[0]).lower() in self.all_funcs:  # or \
        #         # not self.general_handler.are_all_terminal_instances(self.last_expression):
        #     return
        # literal_expression = ''.join([str(x) for x in self.last_expression])
        #
        # if _type == 'numeric':
        #     try:
        #         numeric_expression_resolution = eval(literal_expression)
        #         if isinstance(numeric_expression_resolution, (int, float, str, antlr4.tree.Tree.TerminalNodeImpl)):
        #             if isinstance(numeric_expression_resolution, (int, float)):
        #                 self.values.popitem(last=True)
        #                 self.values[ctx_obj] = numeric_expression_resolution
        #                 self.math_stack_update(ctx_obj, self.values[ctx_obj])
        #                 # self.stack_handler.process_ordered_dict(self.values, literal_expression, self.values[ctx_obj])
        #                 y = 'test'
        #
        #         self.last_expression_result[literal_expression] = \
        #             self.general_handler.remove_unnecessary_nesting(numeric_expression_resolution)
        #         last_key, last_val = next(reversed(self.variables.items()))  # Reference without removing
        #         if not last_val:
        #             self.variables[last_key] = self.last_expression_result[literal_expression]
        #     except Exception as e:
        #         self.generic_processing_error(f'{type(ctx_obj)}::expression_eval()', e)
        # elif _type == 'string':
        #     try:
        #         parameters = self.extract_parameters(self.last_expression)
        #         if not self.general_handler.are_all_terminal_instances(parameters):
        #             return
        #         for index, parameter in enumerate([str(x) for x in parameters]):
        #             x = self.main_df.columns
        #             if parameter in self.main_df.columns:
        #                 parameters[index] = self.general_handler.get_column_entries(self.main_df, parameter)
        #             elif parameter in self.variables:
        #                 parameters[index] = self.variables[parameter]
        #         parameters = [self.string_handler.validate_string_asts(x) for x in parameters]
        #         self.last_expression_result[literal_expression] = \
        #             self.general_handler.remove_unnecessary_nesting(
        #                 self.string_handler.transform_strings(parameters, str(self.last_expression[0])))
        #     except Exception as e:
        #         logging.error(f'[x] String eval failed on the last string expression:\n{self.last_expression}\n\t{e}')
        #         exit(1)
        # elif _type == 'specific':
        #     pass
        # else:
    ...
    @staticmethod
    def generic_processing_error(obj_failure, err_msg):
        logging.error(f'[x] Failure at "{obj_failure}". {err_msg}')
        exit(1)

    # For terminalNodeImpl ONLY
    def math_stack_update(self, ctx_obj, last_val):
        if isinstance(last_val, (int, float)):
            self.collapse(self, self.values)
            self.stack_handler.trim_stack_to_last_list(self.values)
            last_key, last_val = next(reversed(self.values.items()))
            is_already_in_df = False
            if self.general_handler.are_all_terminal_instances(last_val):
                if isinstance(last_val, list):
                    for index, _val in enumerate(last_val):
                        if _val in self.main_df.columns:
                            is_already_in_df = True
                            last_val[index] = self.general_handler.get_column_entries(self.main_df, _val)
                try:
                    ast_validated_last_val = self.math_handler.validate_numeric_asts(last_val)
                    literal_expression = ''.join([str(x) for x in ast_validated_last_val])
                    numeric_expression_resolution = ''
                    if '/' or '*' in literal_expression:
                        numeric_expression_resolution = self.math_handler.perform_mult_div_numpy(ast_validated_last_val)
                    elif '+' or '-' in literal_expression:
                        numeric_expression_resolution = self.math_handler.perform_add_sub_numpy(ast_validated_last_val)
                    if is_already_in_df:
                        # Need process to add result immediately back into the dataframe.
                        # We'll pull all math from df moving forward.
                        pass
                    if isinstance(numeric_expression_resolution, (int, float)):
                        self.values.popitem(last=True)
                        self.values[ctx_obj] = numeric_expression_resolution
                    ...
                    self.last_expression_result[literal_expression] = numeric_expression_resolution
                    last_key, last_val = next(reversed(self.variables.items()))  # Reference without removing
                    if not last_val:
                        self.variables[last_key] = self.last_expression_result[literal_expression]
                except Exception as e:
                    self.generic_processing_error(f'{type(ctx_obj)}::math_stack_update()', e)

            else:
                return

    # For terminalNodeImpl ONLY
    def string_stack_update(self, ctx_obj, last_val):
        if isinstance(last_val, (str, int, float, antlr4.tree.Tree.TerminalNodeImpl)):
            self.collapse(self, self.values)
            self.stack_handler.trim_stack_to_last_list(self.values)
            last_key, last_val = next(reversed(self.values.items()))
            is_already_in_df = False
            if self.general_handler.are_all_terminal_instances(last_val):
                ...
                # try:
                # literal_expression = ''.join([str(x) for x in ast_validated_last_val])
                # last_val = [str(x) for x in last_val]
                # if self.general_handler.are_all_terminal_instances(last_val):
                #     if isinstance(ctx_obj, TQP.StringValueContext):
                #         if str(last_val[0]) == 'eval':
                #             if len(last_val) >= 4:
                #                 self.general_handler.add_or_update_column(self, last_val[1], ''.join(last_val[3:]))

                # string_resolution = ''
                # if '/' or '*' in literal_expression:
                #     string_resolution = self.math_handler.perform_mult_div_numpy(ast_validated_last_val)
                # elif '+' or '-' in literal_expression:
                #     string_resolution = self.math_handler.perform_add_sub_numpy(ast_validated_last_val)
                # if is_already_in_df:
                #     # Need process to add result immediately back into the dataframe.
                #     # We'll pull all math from df moving forward.
                #     pass
                # if isinstance(string_resolution, str):
                #     self.values.popitem(last=True)
                #     self.values[ctx_obj] = string_resolution
                #     self.stack_handler.process_ordered_dict(self.values, literal_expression, self.values[ctx_obj])
                #
                # self.last_expression_result[literal_expression] = string_resolution
                # last_key, last_val = next(reversed(self.variables.items()))  # Reference without removing
                # if not last_val:
                #     self.variables[last_key] = self.last_expression_result[literal_expression]
                # except Exception as e:
                # self.generic_processing_error(f'{type(ctx_obj)}::string_stack_update()', e)

            else:
                return

    def replace_list_in_stack_with_results(self, target_list, replacement_obj, ordered_dict):
        """
        Recursively updates the stack in place with results where lists match the target_list.
        """

        def replace_if_match(target, replacement, container, _key):
            """Replace the list with replacement_obj if it matches target_list."""
            if isinstance(container[_key], list):
                if self.general_handler.are_all_terminal_instances(container[_key]):
                    if container[_key] == target:
                        container[_key] = replacement
                else:
                    for i, item in enumerate(container[_key]):
                        if isinstance(item, list):
                            replace_if_match(target, replacement, container[_key], i)
                        elif hasattr(item, 'children'):
                            replace_in_children(target, replacement, item)

        def replace_in_children(target, replacement, obj):
            """Recursively check and replace in 'children' attribute."""
            if hasattr(obj, 'children'):
                if isinstance(obj.children, list):
                    for i, item in enumerate(obj.children):
                        if isinstance(item, list):
                            replace_if_match(target, replacement, obj.children, i)
                        elif hasattr(item, 'children'):
                            replace_in_children(target, replacement, item)

        for key, value in ordered_dict.items():
            if isinstance(value, OrderedDict):
                self.replace_list_in_stack_with_results(target_list, replacement_obj, value)
            elif isinstance(value, list):
                replace_if_match(target_list, replacement_obj, ordered_dict, key)
            elif hasattr(value, 'children'):
                replace_in_children(target_list, replacement_obj, value)
