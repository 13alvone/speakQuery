...
def resolve_and_import_so(module_dir: str, module_prefix: str) -> ModuleType:
    ...
    if not os.path.isdir(real_module_dir):
        logging.error(f"[x] The module directory {real_module_dir} does not exist or is not a directory")
        raise FileNotFoundError(f"The module directory {real_module_dir} does not exist or is not a directory")
    ...
    try:
        ...
        for entry in os.listdir(real_module_dir):
            ...
            if not (entry.startswith(module_prefix) and entry.endswith(".so")):
                logging.debug(f"[DEBUG] Skipping file '{entry}' as it does not match prefix and suffix criteria")
                continue
            ...
            if not candidate_path.startswith(real_module_dir):
                logging.warning(f"[!] File '{candidate_path}' is not within the specified module directory")
                continue
    ...
    if not valid_so_files:
        logging.error(f"[x] No valid .so files found in '{real_module_dir}' with prefix '{module_prefix}'")
        raise FileNotFoundError(f"No valid .so files found in '{real_module_dir}' with prefix '{module_prefix}'")
    ...
    try:
        # Use the provided module_prefix as the module's name to correctly map to the export function.
